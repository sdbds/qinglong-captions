...
$kimi_api_key = ""
$kimi_model_path = "moonshotai/kimi-k2.6"
...
$vlm_image_model = ""  # Options: "moondream", "qwen_vl_local", "step_vl_local", "penguin_vl_local", "reka_edge_local", "gemma4_local", "lfm_vl_local", ""

# ALM model configuration for audio tasks
$alm_model = ""  # Options: "gemma4_local", "music_flamingo_local", "eureka_audio_local", "acestep_transcriber_local", "cohere_transcribe_local", "mega_asr_local", ""
$audio_task = ""  # Gemma 4 local audio contract: "asr" or "ast"
$gemma4_model_id = ""  # Explicit Hugging Face repo override
...
function Resolve-SelectedRouteModelIds {
  ...
  $RouteSpecs = @(
    @{ name = $ocr_model; explicit = "" },
    @{ name = $vlm_image_model; explicit = "" },
    @{ name = $alm_model; explicit = "" }
  )

  if ($alm_model -eq "gemma4_local" -and $gemma4_model_id) {
    $RouteSpecs[2].explicit = $gemma4_model_id
  }
  elseif ($vlm_image_model -eq "gemma4_local" -and $gemma4_model_id) {
    $RouteSpecs[1].explicit = $gemma4_model_id
  }
  ...
  foreach ($Spec in $RouteSpecs) {
    ...
    $ResolvedId = ""
    if (-not [string]::IsNullOrWhiteSpace($Spec.explicit)) {
      $ResolvedId = $Spec.explicit
    }
    else {
...
if ($audio_task) {
  [void]$ext_args.Add("--audio_task=$audio_task")
}

if ($gemma4_model_id) {
  [void]$ext_args.Add("--gemma4_model_id=$gemma4_model_id")
}
...
if ($alm_model) {
  ...
  if ($alm_model -eq "gemma4_local") {
    Add-UvExtra "gemma4-local"
  }
  elseif ($alm_model -eq "music_flamingo_local") {

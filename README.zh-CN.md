...
当前版本：`4.5.0` · [English README](README.md) · [更新日志](CHANGELOG.md)
...
MuScriptor 通过 `muscriptor-local` profile 安装，支持官方 `small`、`medium`、`large` 模型。只导出 MIDI、JSON 或 JSONL 时不需要音频合成器。启用试听后，运行时会在模型推理前执行预检；FluidSynth 不可用会终止整批任务，因此需要关闭试听才能在没有 FluidSynth 时只导出符号结果。

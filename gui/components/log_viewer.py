...
class LogViewer:
    ...
    def __init__(self, max_lines: int = 1000, height: str = "50vh"):
        ...
        with ui.card().classes(get_classes('card')).style(
            'width: 66vw; max-width: 100%; box-sizing: border-box;'
        ):
            # 工具栏
            with ui.row().classes('w-full items-center justify-between q-pa-md'):
                ...
                with ui.row().classes('items-center gap-2'):
                    ...
                    self.scroll_toggle, self.get_scroll_value = toggle_switch_simple(
                        label=t('auto_scroll', 'Auto Scroll'),
                        value=True,
            ...
            # 日志文本区域 - 现代化样式
            with ui.element('div').classes('w-full q-px-md q-mb-md').style(f'''

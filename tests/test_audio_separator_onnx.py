...
from module.audio_separator import build_parser, build_song_output_dir
...
def test_load_audio_separator_metadata_file_reads_contract(tmp_path):
    ...
    assert metadata.default_segment_size == 1151
    assert metadata.stem_names[3] == "vocals"


def test_segment_size_to_chunk_size_matches_model_formula():
    assert segment_size_to_chunk_size(1151, 512) == 588800


def test_compute_chunk_positions_appends_tail_chunk():
    starts, step_size = compute_chunk_positions(total_samples=1000, chunk_size=400, overlap=4)

...
def _write_siglip2_snapshot(
    ...
    *,
    ...
    if ood_ref:
        np.savez(
            version_dir / f"{stem}_ood_ref.npz",
            mu=np.array([0.0, 1.0], dtype=np.float32),
            cov_inv=np.eye(2, dtype=np.float32),
            p50=np.array(1.0, dtype=np.float32),
            p95=np.array(2.0, dtype=np.float32),
        )
...
def test_siglip2_onnx_bundle_keeps_legacy_constructor_shape():
    ...
    assert bundle.ood_ref_path is None
...
def test_download_cl_tagger_v2_artifacts_uses_explicit_v2_cache_dir(tmp_path, monkeypatch):
    ...
    def fake_snapshot_download(*, repo_id, allow_patterns, local_dir, force_download, token=None):
        ...
        _write_siglip2_snapshot(Path(local_dir), tag_metrics=True, ood_ref=True)
        return local_dir
...
def test_load_cl_tagger_v2_vocabulary_accepts_prefixed_keys(tmp_path):
    vocab_path = tmp_path / "prefixed_vocabulary.json"
    vocab_path.write_text(
        json.dumps(
            {
                "v1_04/idx_to_tag": {"0": "masterpiece", "1": "1girl"},
                "v1_04/tag_to_category": {
                    "masterpiece": "Quality",
                    "1girl": "General",
                },
            }
    ...
    assert vocabulary.names == ["masterpiece", "1girl"]
    assert vocabulary.category_indices["quality"].tolist() == [0]
    assert vocabulary.category_indices["general"].tolist() == [1]

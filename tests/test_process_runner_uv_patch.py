...
def test_sheet_music_musvit_uses_dedicated_uv_extra_by_default(monkeypatch):
    ...
    result = asyncio.run(
        runner.run_python_script("module.sheet_music_musvit", ["--help"], native_console=False),
    )

    assert result.status.value == "成功"
    assert captured == {"extras": ["musvit-onnx"], "groups": []}
    assert commands == [["python", "./module/sheet_music_musvit.py", "--help"]]

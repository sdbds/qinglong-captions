...
- score.py：配对 NoteStartEvent/NoteEndEvent，生成统一 AnalyzedScore。
...
精确样本数根据输入采样率换算，并作为算法版本的一部分写入 metadata。
...
时序分析必须针对 source_path 的原始完整混音，而不是 stem input_path。原因是鼓点和下拍在单一人声/乐器 stem 中可能缺失，各 stem 独立分析也会产生互相矛盾的 tempo map。

### 9.4 MuScriptor 上游 WebUI

上游 /transcribe 当前直接返回上游固定 120/100 的 MIDI，/auralize 直接调用命令名 fluidsynth。项目包装层必须接管：

- 转录完成后的 MIDI 构建，改为通用 timing + dynamics + 本地 MIDI exporter。
- auralize 的 executable resolution，传入解析出的绝对路径。
- 保留现有流式 start/end/progress 协议，避免修改编译后的 Web 前端。
...
- muscriptor-local extra 增加 beat-this==1.1.0、music21==10.5.0。

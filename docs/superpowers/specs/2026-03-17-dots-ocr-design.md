...
v1 不把 “PDF -> Markdown + 图替换成 SVG” 作为正式契约。

`prompt_image_to_svg` 在 v1 只保证单图输入。对 PDF 直接拒绝，不做逐页 SVG 生成，也不发明混合 Markdown/SVG 文档格式。
...
v1 不再要求 `raw` 同时承担 “多页 SVG 索引文本” 这类额外语义。
...
- `dots_ocr` 需要在必要时 override server 路径，以便根据 `prompt_mode` 正确写出 `result.md` 或 `result.svg`
...
- 文本任务写 `result.md`
- SVG 任务写 `result.svg`
- `prompt_image_to_svg` + PDF 直接失败，不生成混合输出
...
因此 `prompt_image_to_svg` 的模型选择和输出写盘规则必须由 `prompt_mode` 统一控制，不能让 backend 自己决定。
...
本设计采用单 provider 方案，把 `dots.ocr-1.5` 作为新的 `dots_ocr` OCR provider 接入现有 Provider V2 架构。任务语义不再由本仓库发明，而是直接对齐官方 demo 的 `prompt_mode` 和官方 `dict_promptmode_to_prompt`。文本与 SVG 的模型选择、输出类型和 direct / server 行为都由 `prompt_mode` 统一驱动；而 “PDF -> Markdown + 图替换 SVG” 这类混合文档输出不作为本版正式契约。整个接入以减少特例、避免双真相源、并保持现有 OCR 路由兼容为原则。

...
3. **生成吞吐：** 原始问题复现时环境未安装 `flash-linear-attention` 和 `causal-conv1d`，Transformers 回退到 Torch 实现；回退路径中的逐步计算使低利用率和长尾时间更明显，但不是不发 EOS 的根因。
...
OvisOCR2 有一个经实测确认的窄例外：当检测到的整个周期去掉空白后严格等于 `1`，且周期本身包含换行，说明模型正在循环生成独占行 `1`。此时删除完整重复区，不保留模型卡算法通常保留的一个周期。普通、未经重复检测证明的单个末行 `1` 必须保留，不能做无上下文的尾字符清理。
...
无论后端，Provider 先从首个独占行的空 `<think>...</think>` 控制块处截断并去掉其前方尾随空白，再采用模型卡周期算法做第二层清理：文本长度至少 8000；周期范围 1 到 200；重复总字符至少 100；重复次数至少 5。通常只删除检测到的截断式重复尾并保留一个完整周期；独占行 `1` 循环采用上述模型专用例外。截断后为空按空模型输出处理，不写入伪结果。
...
`ovis-ocr2` extra 包含 `torch-base`、`transformers[serving]>=5.7.0`、内部共享的 `qwen35-fast-path` 和 `PyMuPDF`。不加入 vLLM、`qwen-vl-utils` 或 `img2pdf`。与旧 Transformers 固定版本及 fork runtime 的 extra 声明冲突隔离。

`qwen35-fast-path` 同时提供 `flash-linear-attention==0.5.1` 与 `causal-conv1d==1.6.2.post1`；Transformers 只有在两者均可导入时才启用 Qwen3.5 Gated DeltaNet 快速路径。该共享 profile 同时由 `ovis-ocr2`、`infinity-parser2-ocr`、`chandra-ocr`、`qwen-vl-local` 和 `marlin-2b-local` 引用。Linux 使用官方包；Windows 仅在 Python 3.11、Torch 2.13、CUDA 13 组合下安装 `triton-windows` 和项目固定的 causal wheel，Python 3.10/3.12 保持可安装并回退到 Torch。

Infinity Parser2、Chandra 和 Qwen 保留 FlashAttention 2 处理视觉编码器及全注意力层；OvisOCR2 和 Marlin 的 extra 不额外安装 FA2，全注意力层沿用各自加载器的现有自动后端。FLA、causal-conv1d、FA2 与停止条件职责不同，不能互相替代。快速内核可能带来数值级生成差异，因此不承诺逐 token 一致或精度提升。
...
- StoppingCriteria 测试断言 batch size 1、返回张量的形状和设备、每页状态隔离、触发元数据、普通周期保留一份、已确认的独占行 `1` 循环全部删除，以及未经重复证明的单个 `1` 保留。
...
- 完成前运行相关 pytest、ruff、`git diff --check`，并在可用 GPU 上做一次真实 Direct 单页冒烟。只有仓库已跟踪 `uv.lock` 时才运行 `uv lock --check`；当前仓库没有锁文件，不为该功能新建锁文件。
...
Windows Python 3.11.11、Torch 2.13.0+cu130 环境另行验证了 `flash-linear-attention==0.5.1`、`causal-conv1d==1.6.2.post1` 和 `triton-windows==3.7.1.post27` 可以同时导入。未启用 UTF-8 模式时已复现 Torch Inductor 模板的 GBK 解码失败；以 `PYTHONUTF8=1` 启动同一解释器后导入成功。该组合现在由共享 `qwen35-fast-path` profile 提供；其他 Windows Python 版本不选择 cp311 wheel 并回退到 Torch。
